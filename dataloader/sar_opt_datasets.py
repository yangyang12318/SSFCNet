...
y_transformer = Compose([ToTensor()])
...
class Dataset_self(data.Dataset):
    def __init__(self, data_dir, label_transform='norm',to_tensor=True):
        super(Dataset_self, self).__init__()

        self.root_path = data_dir
        self.li = ['Image','Image2','label']
    ...
    def __getitem__(self, index):
        ...
        t1_filepath = gt_filepath.replace('label', 'Image')
        t2_filepath = gt_filepath.replace('label', 'Image2')

        img1 = np.asarray(Image.open(t1_filepath).convert('RGB'))
        img2 = np.asarray(Image.open(t2_filepath).convert('RGB'))
        label = np.array(Image.open(gt_filepath).convert('L'))
...
class Dataset_self_64(data.Dataset):
    def __init__(self, data_dir, label_transform='norm',to_tensor=True):
        super(Dataset_self_64, self).__init__()
...
if __name__ == "__main__":
    data_A_dir = r'/data/yy/SAR3/code/E-UNet/data/shuguang/train'
